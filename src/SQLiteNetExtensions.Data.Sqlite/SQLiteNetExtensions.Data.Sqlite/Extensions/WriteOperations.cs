...
public static class WriteOperations
{
	...
	/// </summary>
	/// <param name="conn">SQLite connection object</param>
	...
	public static void Delete(this SqliteConnection conn, object element, bool recursive)
	{
		...
		{
			SqliteConnectionExtensions.Delete(conn, element);
		}
	}
	...
	static void InsertElement(this SqliteConnection conn, object element, bool replace, PropertyInfo? primaryKeyProperty, bool isAutoIncrementPrimaryKey, ISet<object>? objectCache)
	{
		...
		if (shouldReplace)
		{
			SqliteConnectionExtensions.InsertOrReplace(conn, element);
		}
		else
		{
			SqliteConnectionExtensions.Insert(conn, element);
		}
	}
	...
	static void UpdateInverseForeignKeys(this SqliteConnection conn, object element)
	{
		foreach (var relationshipProperty in element.GetType().GetRelationshipProperties())
		{
			var relationshipAttribute = relationshipProperty.GetAttribute<RelationshipAttribute>();
	...
	static void UpdateOneToManyInverseForeignKey(this SqliteConnection conn, object element, PropertyInfo relationshipProperty)
	{
		var type = element.GetType();
		...
		conn.Execute(deleteQuery, [.. deleteParamaters]);
	...
	static void UpdateOneToOneInverseForeignKey(this SqliteConnection conn, object element, PropertyInfo relationshipProperty)
	{
		var type = element.GetType();
	...
	static void UpdateManyToManyForeignKeys(this SqliteConnection conn, object element, PropertyInfo relationshipProperty)
	{
		var type = element.GetType();
		...
		// Return early if any required property is null (defensive check for when assertions are disabled)
		if (currentEntityPrimaryKeyProperty == null || otherEntityPrimaryKeyProperty == null ||
			intermediateType == null || currentEntityForeignKeyProperty == null || otherEntityForeignKeyProperty == null)
		{
			return;
		}
		...
		for (int i = 0; i < loopTo; i++)
		{
			...
			currentChildKeyList.AddRange(
				from object? child in
					conn.Query(conn.GetMapping(intermediateType), currentChildrenQuery, [.. parameters])
				where child != null
		...
		SqliteConnectionExtensions.InsertAll(conn, missingIntermediateObjects);
		...
		for (int i = 0; i < loopTo; i++)
		{
			...
			var parameters = new List<object> { primaryKey! };
			parameters.AddRange(chunk);
			conn.Execute(deleteQuery, [.. parameters]);
		}

	}

	static void DeleteAllIds(this SqliteConnection conn, object[] primaryKeyValues, string entityName, string primaryKeyName)
	{
		...
		else
		{
			...
			{
				conn.DeleteAllIds([.. primaryKeys], entityName, primaryKeyName);
			}
